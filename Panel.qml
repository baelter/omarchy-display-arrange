...
Panel {
  id: root
  moduleName: "baelter.display-arrange"
  ipcTarget: "baelter.display-arrange"
  manageIpc: false
  ...
  IpcHandler {
    target: "baelter.display-arrange"
  ...
  function toggleDisplay(name, enabled) {
    ...
    // `hyprctl keyword monitor` is rejected outright under Hyprland's
    // Lua config parser ("keyword can't work with non-legacy parsers");
    // `hyprctl eval` running the same hl.monitor() call Omarchy's own
    // scaling CLI uses is the live-apply path that still works.
    var lua = enabled
  ...
  // Supersedes stateProc's plain displays line (name/enabled/focused/width/
  // height only) with x/y/scale/transform/refreshRate, needed for the
  ...
  KeyboardPanel {
    ...
    // No fixed cap: with the ARRANGE section added, content can legitimately
    // exceed the old 560 budget for two-plus monitors. fittedContentHeight
    // still bounds this to the actual screen height, so it can't overflow.
            ...
            Item {
              ...
              Repeater {
                ...
                MonitorTile {
                  ...
                  display: modelData
                  areaWidth: layoutArea.width
                  areaHeight: layoutArea.height
                }
              }
            }
          }
  ...
  // Draggable rectangle in the ARRANGE canvas, scaled down to fit the panel.
  // Position only re-syncs from the model while not mid-drag, so the 5s
  // periodic refresh doesn't fight the pointer.
  component MonitorTile: Rectangle {
    ...
    // `bounds` anchors screen-space <-> logical-space translation to the
    // current layout's origin. `capacity` (sum of widths/heights, not the
    // tighter current bounding box) sizes the scale so there's always room
    // to drag into e.g. a full vertical stack, not just rearrange in place.
    readonly property var bounds: Model.computeLayoutBounds(root.displays)
    readonly property var capacity: Model.computeLayoutCapacity(root.displays)
    readonly property var size: Model.logicalSize(display)
    readonly property real fitScale: {
      if (capacity.width <= 0 || capacity.height <= 0) return 1
      var margin = Style.space(8)
      var sx = (areaWidth - margin * 2) / capacity.width
      var sy = (areaHeight - margin * 2) / capacity.height
      return Math.min(sx, sy)
    }
    readonly property real offsetX: (areaWidth - capacity.width * fitScale) / 2
    readonly property real offsetY: (areaHeight - capacity.height * fitScale) / 2

    visible: display.enabled
    width: Math.max(size.width * fitScale, 1)
    height: Math.max(size.height * fitScale, 1)
    radius: Math.min(4, Style.cornerRadius)
    color: Style.hoverFillFor(root.bar.foreground, Color.accent)
    border.width: display.focused ? 2 : 1
    border.color: root.bar.foreground

    function syncFromModel() {
      if (tile.dragging) return
      tile.x = offsetX + (display.x - bounds.minX) * fitScale
      tile.y = offsetY + (display.y - bounds.minY) * fitScale
    }

    Component.onCompleted: syncFromModel()
    onDisplayChanged: syncFromModel()
    onFitScaleChanged: syncFromModel()
    onBoundsChanged: syncFromModel()
    ...
    MouseArea {
      ...
      onReleased: {
        tile.dragging = false
        var rawX = tile.bounds.minX + (tile.x - tile.offsetX) / tile.fitScale
        var rawY = tile.bounds.minY + (tile.y - tile.offsetY) / tile.fitScale
        root.applyLayout(tile.display, rawX, rawY)
      }
    }
  }
